...
from odoo.addons.payment_cod.controllers.main import AirwallexController
...
class PaymentProvider(models.Model):
    ...
    cod_email_account = fields.Char(
        string="Email",
        help="The public business email solely used to identify the account with Airwallex",
        required_if_provider='cod',
        default=lambda self: self.env.company.email,
    )
    cod_client_id = fields.Char(string="Airwallex Client ID", required_if_provider='cod')
    cod_client_secret = fields.Char(string="Airwallex Client Secret", groups='base.group_system')
    cod_access_token = fields.Char(
        string="Airwallex Access Token",
        help="The short-lived token used to access Airwallex APIs",
        groups='base.group_system',
    )
    cod_access_token_expiry = fields.Datetime(
        string="Airwallex Access Token Expiry",
        help="The moment at which the access token becomes invalid.",
        default='1970-01-01',
        groups='base.group_system',
    )
    cod_webhook_id = fields.Char(string="Airwallex Webhook ID")

    # === ACTION METHODS === #

    def action_cod_create_webhook(self):
        ...
        webhook_data = self._cod_make_request('/v1/notifications/webhooks', json_payload=data)
        self.cod_webhook_id = webhook_data.get('id')

    #=== BUSINESS METHODS ===#

    def _cod_make_request(
        ...
        """
        url = self._cod_get_api_url() + endpoint
        ...
        if not is_refresh_token_request:
            headers['Authorization'] = f'Bearer {self._cod_fetch_access_token()}'
        try:
            ...
            try:
                ...
                # Airwallex errors https://developer.cod.com/api/rest/reference/orders/v2/errors/
                _logger.exception(
    ...
    def _cod_fetch_access_token(self):
        ...
        """
        if fields.Datetime.now() > self.cod_access_token_expiry - timedelta(minutes=5):
            response_content = self._cod_make_request(
                '/v1/oauth2/token',
                data={'grant_type': 'client_credentials'},
                auth=(self.cod_client_id, self.cod_client_secret),
            ...
            self.write({
                'cod_access_token': access_token,
                'cod_access_token_expiry': fields.Datetime.now() + timedelta(
                    seconds=response_content['expires_in']
                ),
            })
        return self.cod_access_token
    ...
    def _cod_get_api_url(self):
        ...
        if self.state == 'enabled':
            return 'https://api-m.cod.com'
        else:
            return 'https://api-m.sandbox.cod.com'
    ...
    def _cod_get_inline_form_values(self, currency=None):
