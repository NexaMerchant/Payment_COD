...
class AirwallexController(http.Controller):
    _complete_url = '/payment/cod/complete_order'
    _refund_url = '/payment/cod/refund'
    _webhook_url = '/payment/cod/webhook/'

    @http.route(_refund_url, type='json', auth='public', methods=['POST'])
    def cod_refund(self, provider_id, transaction_id, amount=None):
        ...
        response = provider_sudo._cod_make_request(
            f'/v1/transactions/{transaction_id}/refund', data={'amount': amount}
        )
        normalized_response = self._normalize_cod_data(response)
        tx_sudo = request.env['payment.transaction'].sudo()._get_tx_from_notification_data(
            'cod', normalized_response
        )
        tx_sudo._handle_notification_data('cod', normalized_response)

    @http.route(_complete_url, type='json', auth='public', methods=['POST'])
    def cod_complete_order(self, provider_id, order_id, reference=None):
    ...
    def cod_webhook(self):
        """ Process the notification data sent by Airwallex to the webhook.

        See https://developer.cod.com/docs/api/webhooks/v1/.
        ...
        if data.get('event_type') in const.HANDLED_WEBHOOK_EVENTS:
            normalized_data = self._normalize_cod_data(
            ...
            try:
                ...
                tx_sudo._handle_notification_data('cod', normalized_data)
    ...
    def _normalize_cod_data(self, data, from_webhook=False):
    ...
    def _verify_notification_origin(self, notification_data, tx_sudo):
        ...
        See https://developer.cod.com/docs/api/webhooks/v1/#verify-webhook-signature_post.
        ...
        data = json.dumps({
            ...
            'webhook_id': tx_sudo.provider_id.cod_webhook_id,
            'webhook_event': notification_data,
        })
        verification = tx_sudo.provider_id._cod_make_request(
